...
- Template Wiki (Rider-first): [Chinese Home](https://github.com/alkaid616/ClosureMod/wiki/Home) | [English Home](https://github.com/alkaid616/ClosureMod/wiki/Home-EN).
...
`dotnet new ritsulibmod -n MyMod` generates a project called `MyMod` and renames `ClosureMod`, sample class names, sample resource file names, resource folders, manifest names, namespaces, and localization IDs to match the new name.

### Option B: manual copy

1. Copy the whole directory and rename it to your mod name.
2. Edit `ClosureMod.json` and update `id`, `name`, `author`, and `description`.
3. Edit `ClosureModCode/Entry.cs` and update `ModId`.
4. For a full rename, also update the project name and namespace in `.csproj`, `.sln`, and `project.godot`.
5. Rename the resource directory `ClosureMod/` to your `ModId`, then update the related `Entry.ResPath` paths in code.
...
> **`PackageReference` in `.csproj` only controls compile-time resolution; `dependencies` in `ClosureMod.json` is what the game loader checks at runtime. The template syncs the `STS2-RitsuLib` dependency version to the resolved NuGet version during build, but `min_game_version` still needs manual review.**
...
1. After build, confirm `dependencies[STS2-RitsuLib].version` in `ClosureMod.json` has been synced to the resolved `STS2.RitsuLib` version.
...
| Command | Behavior |
|---|---|
| `dotnet build .\ClosureMod.csproj` | Full build: compile + `CopyMod` + `ExportPCK` |
...
- **`CopyMod`**: copies the DLL and manifest to the game's `mods/ClosureMod` directory.
...
```text
ClosureMod/
├── ClosureModCode/   # C# source
├── ClosureMod/       # Godot resources, localization, and placeholder scenes
├── ClosureMod.csproj
├── ClosureMod.json   # Mod manifest
├── project.godot
└── local.props.template
```

`res://ClosureMod/...` is the Godot/PCK resource path, mapping to the repository resource directory `ClosureMod/`, **not** to the C# namespace. When you create a project from the NuGet template, these directory names, file names, and namespaces are renamed consistently to match the new mod name.
...
| Property | Value |
|---|---|
| Type | `ClosureModCharacter` |
| Expected ID | `CLOSURE_MOD_CHARACTER_CLOSURE_MOD_CHARACTER` |
| Starter deck | 4 × `ClosureModStrike`, 4 × `ClosureModDefend`, 1 × `ClosureModRelic` |
...
|---|---|---|
| `ClosureModStrike` (attack) | character card pool | `CLOSURE_MOD_CARD_CLOSURE_MOD_STRIKE` |
| `ClosureModDefend` (skill) | character card pool | `CLOSURE_MOD_CARD_CLOSURE_MOD_DEFEND` |
| `ClosureModRelic` | `ClosureModRelicPool` | `CLOSURE_MOD_RELIC_CLOSURE_MOD_RELIC` |

### Static placeholder assets

**Images** (`res://ClosureMod/images/...`):

- `cards/ClosureModStrike.png`, `cards/ClosureModDefend.png`: sample card art.
- `relics/ClosureModRelic.png`: sample relic icon.
- `characters/ClosureMod_character_*.png`: character icons, select art, map marker, and energy icons.

**Scenes** (`res://ClosureMod/scenes/characters/...`):

| Scene | Purpose | Placeholder structure |
|---|---|---|
| `ClosureMod_character.tscn` | Combat character | `%Visuals`, `%Bounds`, `%IntentPos`, `%CenterPos`, `%TalkPos` |
| `ClosureMod_energy_counter.tscn` | Energy counter | `%EnergyVfxBack`, `%Layers`, `%RotationLayers`, `%EnergyVfxFront`, `Label` |
| `ClosureMod_merchant.tscn` | Merchant | — |
| `ClosureMod_rest_site.tscn` | Rest site | `%ControlRoot`, `%SelectionReticle`, `%Hitbox`, `%ThoughtBubbleRight`, `%ThoughtBubbleLeft` |
| `ClosureMod_character_select_bg.tscn` | Character select background | — |
...
`ClosureMod.json` is the mod manifest. The game loader reads it at startup to identify the mod, check dependencies, and decide whether to load. Full example:
